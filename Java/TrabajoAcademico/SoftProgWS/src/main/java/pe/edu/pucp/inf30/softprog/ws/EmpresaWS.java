...
public class EmpresaWS {
    ...
    public String eliminarEmpresa(@WebParam(name = "id")int id)  throws IOException, InterruptedException {
        String url = this.urlBase + "/" + this.NOMBRE_RESOURCE + "/" + id;
        HttpRequest request = HttpRequest.newBuilder()
                .uri(URI.create(url))
                .DELETE()
        ...
        if(statusCode == 204){
            return ("Empresa ID " + id + " eliminada exitosamente (HTTP 204).");
        }
        if (statusCode == 404) {
            String mensajeError = "Error 404: Empresa con ID " + id + " no encontrada.";
            throw new RuntimeException(mensajeError);
        }       
        if (statusCode >= 400) {
            throw new RuntimeException("Fallo en el servicio. Código HTTP: " + statusCode + ". Detalle: " + response.body());
        }
