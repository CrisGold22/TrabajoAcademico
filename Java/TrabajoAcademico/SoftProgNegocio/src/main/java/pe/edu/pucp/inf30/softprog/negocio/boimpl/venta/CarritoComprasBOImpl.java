    ...
    public void insertar(CarritoCompras modelo) {
        ...
        try(Connection conn = dbManager.getConnection()){
            ...
            try{
                ...
                for(LineaCarrito linea : modelo.getLineasCarrito()){
...
//        List<LineaCarrito> lista = carrito.getLineasCarritos();
        return lineaCarrito.listarPorIdCarrito(idCarritoCompras);
    }

    @Override
    public CarritoCompras obtenerCarritoComprasPorIdCliente(int idCliente) {
        if (idCliente <= 0) {
            throw new IllegalArgumentException("ID de carritoCompras inválido");
        } 
        CarritoCompras carrito = carritoCompras.obtenerCarritoPorIdCliente(idCliente);
        List<LineaCarrito> lineas = listarLineaCarritoPorIdCarrito(carrito.getId());
