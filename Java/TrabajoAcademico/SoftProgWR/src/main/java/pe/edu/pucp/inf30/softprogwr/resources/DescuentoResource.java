...
public class DescuentoResource {
 ...
 //       this.ordenCompraBO=new OrdenCompraBOImpl();
    ...
    @PUT
    @Path("actualizarDescuentoPorIdProducto/{id_producto}")
    public Response actualizarDescuentoPorIdProducto(@PathParam("id_producto") int id_producto, Descuento descuento){
        if(this.productoBO.obtener(id_producto)==null){
            return Response.status(Response.Status.NOT_FOUND)
                    .entity("producto: " + id_producto + ", no encontrado"+" y el descuento"+descuento.getPrecioPorVolumen()+"no se puede aplicar")
                    .build();
        }
        this.descuentoBO.actualizarDescuentoPorIdProducto(id_producto, descuento);
        return Response.ok(descuento).build();
    }
