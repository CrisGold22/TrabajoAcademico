...
public class CarritoComprasDAOImpl extends TransaccionalBaseDAO<CarritoCompras> implements CarritoComprasDAO  {
    ...
    protected PreparedStatement comandoObtenerPorCliente(Connection conn, int id) throws SQLException{
    ...
    @Override
    public CarritoCompras obtenerPorCliente(int id){
        return ejecutarComando(conn ->obtenerPorCliente(id, conn));
    }
    @Override
    public CarritoCompras obtenerPorCliente(int id, Connection conn){
        try(PreparedStatement cmd = this.comandoObtenerPorCliente(conn, id)){
