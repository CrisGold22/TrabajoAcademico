...
CREATE PROCEDURE insertarEmpresaValidandoCodigoPostal (
    IN p_ruc             VARCHAR(11),
    IN p_razonsocial     VARCHAR(200),
    IN p_direccionFiscal VARCHAR(255),
    IN p_departamento    VARCHAR(100),
...
BEGIN
    DECLARE v_codigo_postal_count INT DEFAULT 0;
    DECLARE v_codigo_postal_id INT DEFAULT NULL;
    
    SELECT idCodigoPostal INTO v_codigo_postal_id
    FROM CodigoPostal 
    WHERE codigo = p_codigoPostal AND activo = 1;
    
    IF v_codigo_postal_id IS NOT NULL THEN
        SET v_codigo_postal_count = 1;
    ELSE
        SET v_codigo_postal_count = 0;
    END IF;
    
    IF v_codigo_postal_count = 0 THEN
        SET p_id = -1;
    ELSE
        INSERT INTO Empresa (
            ruc, razonSocial, direccionFiscal, departamento, provincia, distrito,
