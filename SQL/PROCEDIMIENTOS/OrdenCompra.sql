...
DROP PROCEDURE IF EXISTS DesactivarOrdenCompraPorHorario;
DROP PROCEDURE IF EXISTS listarOrdenesCompraPorCuenta;
DROP PROCEDURE IF EXISTS reporteOrdenCompra;
DROP PROCEDURE IF EXISTS reporteDetalleOrdenCompra;
...
CREATE PROCEDURE DesactivarOrdenCompraPorHorario(
    IN p_IdPedido INT,
    OUT p_Mensaje VARCHAR(200)
)
BEGIN
    ...
    DECLARE v_HoraCreacion TIME;
    DECLARE v_FechaLimite DATETIME;
    DECLARE v_LineasAfectadas INT DEFAULT 0;
    DECLARE v_StockActualizado INT DEFAULT 0;
    DECLARE v_ErrorCode CHAR(5) DEFAULT '00000';
    DECLARE v_ErrorMsg TEXT;
    ...
    SELECT estado, activo, fechaCreacion INTO v_Estado, v_Activo, v_FechaCreacion 
    FROM OrdenCompra 
    WHERE idOrdenCompra = p_IdPedido;
    ...
    ELSE
        ...
        IF v_HoraCreacion >= '20:00:00' THEN
            SET v_FechaLimite = DATE_ADD(DATE(v_FechaCreacion), INTERVAL 1 DAY);
            SET v_FechaLimite = CONCAT(DATE(v_FechaLimite), ' 07:00:00');
        ELSE
            SET v_FechaLimite = DATE_ADD(v_FechaCreacion, INTERVAL 4 HOUR);
        END IF;
        
        IF NOW() > v_FechaLimite THEN
            SET p_Mensaje = CONCAT('Error: Tiempo de cancelación expirado. Límite: ', 
                                   DATE_FORMAT(v_FechaLimite, '%d/%m/%Y %H:%i:%s'));
        ELSE
            -- Proceder con la cancelación si está dentro del plazo
            START TRANSACTION;
            ...
            ELSE
                ...
                ELSE
                    COMMIT;
                    SET v_StockActualizado = v_LineasAfectadas;
                    SET p_Mensaje = CONCAT('Éxito: Orden cancelada dentro del plazo permitido. ');
                END IF;
            END IF;
        END IF;
    END IF;
