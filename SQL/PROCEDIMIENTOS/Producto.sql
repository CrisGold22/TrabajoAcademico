...
DROP PROCEDURE IF EXISTS listaProductosPorCategoria;
...
CREATE PROCEDURE insertarProducto (
    ...
    IN p_ImagenProducto                        VARCHAR(150),
...
BEGIN
    INSERT INTO Producto (
        nombre, sku, descripcion,imagenProducto, precioRegular, precioAlMayor,
        unidadDeMedida, stockDisponible, stockMaximo, activo,
        categoriaProducto_idCategoriaProducto,marca
    ) VALUES (
        p_Nombre, p_SKU, p_Descripcion,p_ImagenProducto, p_precioRegular, p_precioAlMayor,
...
CREATE PROCEDURE listaProductosPorCategoria(IN p_nombreCategoria VARCHAR(45))
BEGIN 
	SELECT p.*
    FROM Producto p
    INNER JOIN CategoriaProducto c ON c.idCategoriaProducto = p.categoriaProducto_idCategoriaProducto
